defmodule Binance.Futures.Coin.WebSocket.WSClient do
  ...
  defmodule A do
    use Binance.Coin.WebSocket.WSClient
  end

  # Public channels/streams
  A.start_link(%{name: :"btcusdt-depth-stream", public_channels: ["btcusd_perp@depth@100ms"]})
    ...
    quote do
      ...
      def start_link(args \\ %{}) do
        ...
        if require_auth == true do
          {:ok, %{"listenKey" => listen_key}} = Binance.Coin.create_listen_key(%{}, config)
      ...
      def handle_connect(_conn, state) do
        :ok = info("Binance Coin Futures Connected!")
      ...
      def handle_info({:manual_close}, state) do
        :ok = info("Manual close Binance Coin Futures WS. #{inspect(state)}")
        {:close, state}
      end
      ...
      def handle_disconnect(resp, state) do
        :ok = info("Binance Coin Futures Disconnected! #{inspect(resp)}")
        {:ok, state}
      end
